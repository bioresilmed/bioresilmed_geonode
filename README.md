...
GeoNode project generated with [geonode-project](https://github.com/GeoNode/geonode-project).
...
├─ src/
│  └─ bioresilmed_geonode/      # The actual Django/GeoNode project
│     └─ README.md              # Detailed customization guide (templates, styles, i18n, docker cmds)
└─ docker-compose.yml
...
For all frontend customization and operations, **read the detailed guide here**:
...
- [Bioresilmed Geonode](#bioresilmed-geonode)
  - [Repo layout](#repo-layout)
  - [Quick start](#quick-start)
  - [More docs](#more-docs)
  - [NEXT COMES THE README OF THE geonode-project template...](#next-comes-the-readme-of-the-geonode-project-template)
- [Bioresilmed Geonode](#bioresilmed-geonode-1)
...
## Quick Docker Start
  ...
  ```bash
    ...
    source ~/.venvs/bioresilmed_geonode/bin/activate
...
**NOTE**: *You can call your geonode project whatever you like **except 'geonode'**. Follow the naming conventions for python packages (generally lower case with underscores (``_``). In the examples below, replace ``bioresilmed_geonode`` with whatever you would like to name your project.*
...
1. Generate the project
    ...
    mkvirtualenv --python=/usr/bin/python3 bioresilmed_geonode
    pip install Django==3.2.16

    django-admin startproject --template=./geonode-project -e py,sh,md,rst,json,yml,ini,env,sample,properties -n monitoring-cron -n Dockerfile bioresilmed_geonode

    cd bioresilmed_geonode
    ```
...
```bash
SOURCE_URL=$SOURCE_URL TARGET_URL=$TARGET_URL ./bioresilmed_geonode/br/backup.sh $BKP_FOLDER_NAME
```
...
```bash
docker exec -it django4bioresilmed_geonode sh -c 'SOURCE_URL=$SOURCE_URL TARGET_URL=$TARGET_URL ./bioresilmed_geonode/br/backup.sh $BKP_FOLDER_NAME'
```

### Run a Restore

```bash
SOURCE_URL=$SOURCE_URL TARGET_URL=$TARGET_URL ./bioresilmed_geonode/br/restore.sh $BKP_FOLDER_NAME
```
...
```bash
docker exec -it django4bioresilmed_geonode sh -c 'SOURCE_URL=$SOURCE_URL TARGET_URL=$TARGET_URL ./bioresilmed_geonode/br/restore.sh $BKP_FOLDER_NAME'
```
