...
RUN mkdir -p /usr/src/bioresilmed_geonode
...
COPY src /usr/src/bioresilmed_geonode/
WORKDIR /usr/src/bioresilmed_geonode
...
RUN chmod +x /usr/src/bioresilmed_geonode/tasks.py \
    && chmod +x /usr/src/bioresilmed_geonode/entrypoint.sh
...
# ENTRYPOINT /usr/src/bioresilmed_geonode/entrypoint.sh
